...
import { DropdownMui } from "../../components/DropDownMUI";
 const SignUpFormToConnect = (props:any) => {
  ...
  const handleEmploymentStatusChange = (status: string) => {
    ...
    console.log('Employment Status in Parent:', status);
  };
  const submitHandler = handleSubmit(async(data:SignUpProps)=>{
    console.log(data)
   
    if (Object.keys(errors).length === 0) {
     const response = await  props.createduser(data,()=>console.log("inserted")); 
        navigate("/login")
